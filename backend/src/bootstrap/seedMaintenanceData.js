...
// Uses upsertUser pattern from team's seed
// ============================================

// Helper to create or update a user
async function upsertMaintenanceUser({ email, password, role, name, studentId, contactNumber }) {
...
async function seedMaintenanceData() {
  ...
  // Students use 'passenger' role since they are also potential ride passengers
  const student1 = await upsertMaintenanceUser({
    email: 'kasun@university.edu',
    password: 'password123',
    role: ROLES.PASSENGER,
    name: 'Kasun Perera',
    studentId: 'IT23000001',
    contactNumber: '+94-77-100-0001',
  });

  const student2 = await upsertMaintenanceUser({
    email: 'nimali@university.edu',
    password: 'password123',
    role: ROLES.PASSENGER,
    name: 'Nimali Fernando',
    studentId: 'IT23000002',
    contactNumber: '+94-77-100-0002',
  });

  // ---- CREATE TECHNICIAN USERS ----
  const tech1 = await upsertMaintenanceUser({
    email: 'nimal@university.edu',
    password: 'password123',
    role: ROLES.STAFF,
    name: 'Nimal Silva',
    contactNumber: '+94-77-200-0001',
  });

  const tech2 = await upsertMaintenanceUser({
    email: 'ruwan@university.edu',
    password: 'password123',
    role: ROLES.STAFF,
    name: 'Ruwan Fernando',
    contactNumber: '+94-77-200-0002',
  });

  const tech3 = await upsertMaintenanceUser({
    email: 'chaminda@university.edu',
    password: 'password123',
    role: ROLES.STAFF,
    name: 'Chaminda Jayasinghe',
    contactNumber: '+94-77-200-0003',
  });

  // ---- CREATE MAINTENANCE ADMIN ----
  const maintenanceAdmin = await upsertMaintenanceUser({
    email: 'sarah@university.edu',
    password: 'password123',
    role: ROLES.ADMIN,
    name: 'Sarah M.D.',
    contactNumber: '+94-77-300-0001',
  });
